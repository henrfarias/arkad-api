import {
  ...
  filterSelicTreasure,
  parseTreasure
} from 'src/scripts/download_selic_treasure_papers/get_treasure_paper'

describe('Script to download, filter, format and persist treasure paper registers', () => {
  ...
  describe('readStream -> FILTER -> format -> persist', () => {
    it('should return a list of "Tesouro Selic" treasure papers', () => {
      // Given
  ...
  describe('readStream -> filter -> FORMAT -> persist', () => {
    test('should format the treasure paper to a new format', () => {
      ...
      const sut = parseTreasure
      const result = sut(treasure)
      expect(result).toStrictEqual(expected)
    })
    describe('readStream -> filter -> format -> PERSIST', () => {})
  })
